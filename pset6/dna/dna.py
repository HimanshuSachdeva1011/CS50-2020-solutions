...
dna = dnalist[0]
# create a dictionary where to store the sequences intended to count
sequences = {}
...
for key in sequences:
    ...
    for i in range(len(dna)):
        ...
        # if the segment of dna corresponds to the key and there is a repetition of it
        if dna[i: i + l] == key:
...
# iterate trough the database of people treating each one like a dict so it can compare to the sequences
with open(argv[1], newline='') as peoplefile:
    people = DictReader(peoplefile)
    for person in people:
        match = 0
        # compares the sequences to every person and prints name if there is a match
